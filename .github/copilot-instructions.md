...
- Key modules:
  - `mcp-server/server.py`: MCP server with 2 company earnings tools:
...
- Local MCP server:
  - **Docker**: `docker build -t mcp-finance-server -f mcp-server/Dockerfile mcp-server/ && docker run -p 3000:3000 -e ALPHAVANTAGE_API_KEY="your_key" mcp-finance-server`
  - **Direct Python**: `cd mcp-server && $env:ALPHAVANTAGE_API_KEY="your_key" && uv run python server.py`
